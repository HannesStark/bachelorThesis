...
label of a piece of data gives us a distribution $P$ with absolute cetrainty and $H(P)=0$. With the
inaccurate distribution $Q$ that the model estimates $H(P,Q)$ will be greater than zero unless
$P=Q$ where $H(P,Q)=H(P,P)=H(P)=0$. So the lerning algorithm can try to minimize $H(P,Q)$.
...
the KL divergence is greater than zero proportional to how differten $Q$ and $P$ are.
...
\paragraph{The Loss Function}\mbox{}\\
