...
z = tf.keras.layers.Conv2D(512, strides=2, kernel_size=3, padding="same", activation="selu")(z)
...
x = tf.keras.layers.Dense(2 * 2 * 512)(decoder_inputs)
x = tf.keras.layers.Reshape([2, 2, 512])(x)
x = tf.keras.layers.Conv2DTranspose(256, kernel_size=3, strides=2, padding="same", activation="selu")(x)
