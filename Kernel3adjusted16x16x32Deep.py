...
latent_dim = 1024
#file_tag = "16x16x32DEEPTRAINEDON178000"
#file_tag = "16x16x32DEEPTRAINEDON178000" + "_dim" + str(latent_dim)
...
z = tf.keras.layers.Conv2D(16, kernel_size=3, padding="same", activation="selu")(z)
z = tf.keras.layers.Conv2D(16, kernel_size=3, padding="same", activation="selu")(z)
z = tf.keras.layers.Conv2D(16, kernel_size=3, padding="same", activation="selu")(z)
...
x = tf.keras.layers.Conv2DTranspose(16, kernel_size=3, padding="same", activation="selu")(x)
x = tf.keras.layers.Conv2DTranspose(16, kernel_size=3, padding="same", activation="selu")(x)
x = tf.keras.layers.Conv2DTranspose(16, kernel_size=3, padding="same", activation="selu")(x)
...
def tsne_vis():
    ...
    images = []
    file_names = os.listdir("RGB-From-Track1128x128split8")
    for filename in file_names[0:3000]:
    ...
    embedded = TSNE(perplexity=7).fit_transform(latent_vars)
